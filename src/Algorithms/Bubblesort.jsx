...
export const bubbleSort = (tempArr, speed) => {
	...
	let count = 0;		//tracks the delay for the animations
	...
	for (let i = 0; i < arr.length - 1; i++) {
		...
		for (let j = 0; j < arr.length - i - 1; j++) {
			// visually highlights the two bars being compared
			setTimeout(() => {
			...
			if (arr[j] > arr[j + 1]) {
				// swap the heights   and Changes the background color of the bars to indicate that a swap is happening.
				setTimeout(() => {
				...
				// also make changes in the array
				let temp = arr[j];
				arr[j] = arr[j + 1];
				arr[j + 1] = temp;
			}
